...
class OpenScenario(BasicScenario):
    ...
    def _create_init_behavior(self):
        ...
        # looking for UserDefinedAction
        for i, user_defined_action in enumerate(self.config.init.find('Actions').iter('UserDefinedAction')):
            behavior = OpenScenarioParser.get_custom_behavior(user_defined_action, None, None, None)
            init_behavior.add_child(oneshot_with_check(variable_name="InitCustuomBehavior" + '_%d' % i, behaviour=behavior))
