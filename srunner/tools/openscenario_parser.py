...
class OpenScenarioParser(object):
    ...
    @staticmethod
    def get_custom_behavior(user_defined_action, actor, actor_list=None, catalogs=None):
        maneuver_name = user_defined_action.attrib.get('name', 'unknown')
        ...
        elif user_defined_action.find('CustomAtomicAction') is not None:
            custom_action = user_defined_action.find('CustomAtomicAction')
            
            # parse properties
            properties = custom_action.find('Properties')
            action_name = None
            args = {}
            for prop in properties:
                if prop.attrib.get('name') == 'action_name':
                    action_name = prop.attrib.get('value')
                else:
                    args[prop.attrib.get('name')] = prop.attrib.get('value')
            
            if action_name is None:
                raise AttributeError("CustomAtomicAction requires an action_name property")
            
            # Dynamic import of the atomic
            behavior_cls = getattr(importlib.import_module("srunner.scenariomanager.scenarioatomics.atomic_behaviors"), action_name, None)
            if behavior_cls is None:
                raise AttributeError(f"CustomAtomicAction {action_name} is not a valid atomic behavior")
            
            atomic = behavior_cls(actor, **args, name=maneuver_name)
        return atomic
    ...
    def convert_maneuver_to_atomic(action, actor, actor_list, catalogs):
        ...
        if action.find('GlobalAction') is not None:
            ...
            atomic = OpenScenarioParser.get_custom_behavior(user_defined_action, actor, actor_list, catalogs)
